...
class Settings(BaseSettings):
    ...
    HF_TOKEN: Optional[str] = Field(
        default=None,
        description="Hugging Face User Access Token for authenticated downloads"
    )
    HF_HUB_DISABLE_SYMLINKS_WARNING: str = Field(
        default="1",
        description="Disable HF symlinks warning on Windows"
    )
    ...
    SEMANTIC_SCHOLAR_RATE_LIMIT_COOLDOWN_SECONDS: int = 60
    ARXIV_LIMIT: int = 30
    PUBMED_LIMIT: int = 30
    ...
    PDF_DOWNLOAD_MAX_RETRIES: int = 1
    PDF_DOWNLOAD_TIMEOUT_SECONDS: float = 6.0
    PDF_DOWNLOAD_DIRECT_FIRST: bool = True

    # Full-text enrichment
    ENABLE_FULL_TEXT_DOWNLOAD: bool = True
    FULL_TEXT_DOWNLOAD_LIMIT: int = 5
    FULL_TEXT_DOWNLOAD_BUDGET_SECONDS: float = 15.0
    MIN_EXTRACTED_PDF_TEXT_CHARS: int = 300

    # LLM gap synthesis budget (number of candidates to generate deep LLM dossiers for)
    LLM_GAP_SYNTHESIS_MAX_CANDIDATES: int = 3
    ...
    def ensure_directories(self) -> None:
        ...
        # Export Hugging Face token and suppress Windows symlink warnings
        hf_tok = self.HF_TOKEN or os.environ.get("HF_TOKEN")
        if hf_tok:
            self.HF_TOKEN = hf_tok
